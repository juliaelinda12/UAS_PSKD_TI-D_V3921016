...
demo = cv2.imread("D:/JULIA/image.jpg", 0)
r, c = demo.shape
key = np.random.randint(0, 256, size=(r, c), dtype=np.uint8)  # Generate random key image
cv2.imwrite("D:/JULIA/key.jpg", key)   # Save key image
...
cv2.imwrite("D:/JULIA/enkripsiIMAGE.jpg", encryption)  
   # Save the encrypted image
decryption = cv2.bitwise_xor(encryption, key)  # decrypt
cv2.imwrite("D:/JULIA/deskripsiIMAGE.jpg", decryption) # Save the decrypted image
